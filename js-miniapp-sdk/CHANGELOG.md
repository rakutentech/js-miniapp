...
- **Feature:** `launchInteractiveBrowser` now supports passing an options object (`LaunchBrowserOptions`) with `httpMethod`, `httpBody`, `audience`, and `scopes`, matching the same signature as `launchInternalBrowser`.
