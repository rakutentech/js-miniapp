...
4. Complete all steps in the [Checklist](#checklist)
...
- [x] I ran `yarn sample prettify` `yarn sample build` without issues.

## Coding Guidelines
* All features or bug fixes **must be tested** by one or more unit tests/specs
* All public API methods **must be documented** in the JSDOC and potentially in the user guide.

## Commit messages
Each commit message consists of a header, an optional body and an optional footer. The header has a special format that includes a type and a subject:
...
All changes will be documented using a [CHANGELOG](../js-miniapp-sdk/CHANGELOG.md) by writing chronologically for each version with the following format:
